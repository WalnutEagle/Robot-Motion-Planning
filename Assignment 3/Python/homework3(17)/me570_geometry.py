...
def line_linspace(a_line, b_line, t_min, t_max, nb_points):
    """
    Generatesadiscretenumberof nb_pointspointsalongthecurve    (t=( a1t+ b1, a2t+ b2)  R2fortrangingfrom tMinto tMax
    """
...
class Grid:
    """
    Afunctiontostorethecoordinatesofpointsona2Dgridandevaluate    arbitraryfunctionsonthosepoints
    """
...
class Torus:
    ...
    def phi_push_curve(self, a_line, b_line):
        """
        Thisfunctionevaluatesthecurvext= phi_torus( phit )  R3at nb_points31points        generatedalongthecurvephit usingline_linspaceLinelinspacewith tMin0and tMax1
    ...
    def plot_curves(self):
        ...
        - 34pi0        - 34pi34pi        - -34pi34pi        - 0-34pi and bnparray[[-1,[-1]).
...
class Polygon:
    ...
    def flip(self):
        """
        Reversetheorderofthevertices(ie, transformthepolygonfrom        filledintohollowandviceversa.
        """
        self.vertices = np.fliplr(self.vertices)
    ...
    def is_visible(self, idx_vertex, test_points):
        """
        Checkswhetherapointpisvisiblefromavertexvofapolygon In        ordertobevisible twoconditionsneedtobesatisfied
         - Thepointpshouldnotbeselfoccludedwithrespecttothevertex        v\ (seePolygonis_self_occluded.
         - Thesegmentp-vshouldnotcollidewith anyoftheedgesofthe        polygon(seeEdgeis_collision.
        """
        nb_test_points = test_points.shape[1]
        nb_vertices = self.vertices.shape[1]
    ...
    def is_collision(self, test_points):
        """
        Checkswhethertheapointisincollsionwithapolygon(thatis
        insideforafilledinpolygon andoutsideforahollowpolygon. In        thecontextofthishomework thisfunctionisbestimplementedusing        Polygonis_visible
        """
        flag_points = [False] * test_points.shape[1]
        # We iterate over the polygon vertices, and process all the test points
        # in parallel
...
class Edge:
    ...
    def is_collision(self, edge):
        """
         Returns Trueifthetwoedgesintersect  Note ifthetwoedges        overlapbutarecolinear ortheyoverlaponlyatasingleendpoint
        theyarenotconsideredasintersecting(ie, inthesecasesthe        functionreturns False. Ifoneofthetwoedgeshaszerolength the        functionshouldalwaysreturntheresultthatedgesare        nonintersecting
        """
...
def angle(vertex0, vertex1, vertex2, angle_type='unsigned'):
    """
    Computetheanglebetweentwoedges vertex0- vertex1and vertex0-
    vertex2havinganendpointincommon Theangleiscomputedbystarting    fromtheedge vertex0- vertex1 andthen``walking' ina    counterclockwisemanneruntiltheedge vertex0- vertex2isfound
    """
    # tolerance to check for coincident points
    tol = 2.22e-16
...
class Sphere:
    ...
    def distance(self, points):
        ...
        """
        # Calculate the Euclidean distance between each point and the center of the sphere.
        dist_to_center = np.linalg.norm(points - self.center, axis=0)
        
        if self.radius > 0:  # Filled-in sphere
            # Distance is negative inside the sphere, positive outside.
            d_points_sphere = dist_to_center - abs(self.radius)
        else:  # Hollow sphere
            # Distance is negative outside the sphere, positive inside.
            d_points_sphere = abs(self.radius) - dist_to_center
    ...
    def distance_grad(self, points):
        ...
        """
        # Calculate the gradient of the signed distance.
        # If a point is at the center, return the zero vector.
        dist_to_center = points - self.center
        dist_norm = np.linalg.norm(dist_to_center, axis=0)
        
        if dist_norm == 0:
            return np.zeros_like(points)
        
        if self.radius > 0:  # Filled-in sphere
            grad_d_points_sphere = dist_to_center / dist_norm
        else:  # Hollow sphere
            grad_d_points_sphere = -dist_to_center / dist_norm
